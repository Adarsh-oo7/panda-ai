---
summary: "Agent-I on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Agent-I on a Raspberry Pi
  - Running Agent-I on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Agent-I on Raspberry Pi

## Goal

Run a persistent, always-on Agent-I Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
Agent-I config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/Agent-I-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
export NODE_COMPILE_CACHE=/var/tmp/Agent-I-compile-cache
mkdir -p /var/tmp/Agent-I-compile-cache
export Agent-I_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `Agent-I_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Agent-I, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit Agent-I
```

```ini
[Service]
Environment=Agent-I_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/Agent-I-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Agent-I state/cache on SSD-backed storage to avoid SD-card
...
Most Agent-I features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled Agent-I

# Enable if not
sudo systemctl enable Agent-I

# Start on boot
sudo systemctl start Agent-I
```
...
```bash
# Check logs
journalctl -u Agent-I --no-pager -n 100

# Common fix: rebuild
cd ~/Agent-I  # if using hackable install
npm run build
sudo systemctl restart Agent-I
```
