...
IMAGE_NAME="Agent-I-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  home_dir=$(mktemp -d "/tmp/Agent-I-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.Agent-I/extensions/demo-plugin"

  cat > "$HOME/.Agent-I/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.Agent-I/extensions/demo-plugin/Agent-I.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/Agent-I-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@Agent-I/demo-plugin-tgz",
  "version": "0.0.1",
  "Agent-I": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$pack_dir/package/Agent-I.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/Agent-I-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@Agent-I/demo-plugin-dir",
  "version": "0.0.1",
  "Agent-I": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$dir_plugin/Agent-I.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/Agent-I-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@Agent-I/demo-plugin-file",
  "version": "0.0.1",
  "Agent-I": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/Agent-I.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
