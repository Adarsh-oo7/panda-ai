...
# Agent-I Installer for macOS and Linux
# Usage: curl -fsSL --proto =https --tlsv1.2 https://agenti-ai.com/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Agent-I."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://agenti-ai.com/install.ps1 | iex"
        exit 1
    fi
...
Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://agenti-ai.com/install.sh | bash -s -- [options]
...
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://agenti-ai.com/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://agenti-ai.com/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://agenti-ai.com/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://agenti-ai.com/install.sh | bash"
}
...
main() {
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://agenti-ai.com/install.sh | bash -s -- --install-method npm"
