...
read_when:
  - Designing workspace memory (~/.Agent-I/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Agent-I integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.Agent-I/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.Agent-I/workspace` as canonical human-readable memory.
...
```
~/.Agent-I/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Agent-I**, but keep a separable core library.

### Why integrate into Agent-I?

- Agent-I already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `Agent-I memory recall "…" --k 25 --since 30d`
  - `Agent-I memory reflect --since 7d`
...
Pragmatic take for `~/.Agent-I/workspace`:
