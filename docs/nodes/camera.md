...
Agent-I supports **camera capture** for agent workflows:
...
```bash
Agent-I nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
Agent-I nodes camera snap --node <id> --facing front
Agent-I nodes camera clip --node <id> --duration 3000
Agent-I nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`Agent-I.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `Agent-I` CLI to invoke camera commands on the macOS node.

Examples:

```bash
Agent-I nodes camera list --node <id>            # list camera ids
Agent-I nodes camera snap --node <id>            # prints MEDIA:<path>
Agent-I nodes camera snap --node <id> --max-width 1280
Agent-I nodes camera snap --node <id> --delay-ms 2000
Agent-I nodes camera snap --node <id> --device-id <id>
Agent-I nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
Agent-I nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
Agent-I nodes camera clip --node <id> --device-id <id>
Agent-I nodes camera clip --node <id> --no-audio
```

Notes:

- `Agent-I nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
Agent-I nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
