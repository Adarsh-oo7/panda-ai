...
- `gateway`/`node` approvals are controlled by `~/.Agent-I/exec-approvals.json`.
...
- Agent-I sets `Agent-I_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Agent-I prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`Agent-I security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `Agent-I doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
