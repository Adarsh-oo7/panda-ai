...
This document explains how Agent-I manages sessions end-to-end:
...
Agent-I is designed around a single **Gateway process** that owns session state.
...
Agent-I persists sessions in two layers:
...
- Transcripts: `~/.Agent-I/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Agent-I resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Agent-I reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.Agent-I/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Agent-I intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Agent-I consumes the events, but Pi decides when to compact).
...
Agent-I also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Agent-I bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Agent-I leaves it alone.
...
- `Agent-I status` (CLI)
- `Agent-I sessions` / `sessions --json`
...
Agent-I supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Agent-I strips/suppresses this in the delivery layer.

As of `2026.1.10`, Agent-I also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Agent-I uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Agent-I’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `Agent-I status`.
