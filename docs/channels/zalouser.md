...
read_when:
  - Setting up Zalo Personal for Agent-I
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Agent-I.
...
- Install via CLI: `Agent-I plugins install @Agent-I/zalouser`
- Or from a source checkout: `Agent-I plugins install ./extensions/zalouser`
...
- `Agent-I pairing list zalouser`
- `Agent-I pairing approve zalouser <code>`
...
- On startup, Agent-I resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
Accounts map to `zalouser` profiles in Agent-I state. Example:
...
- Agent-I sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Agent-I sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `Agent-I channels logout --channel zalouser && Agent-I channels login --channel zalouser`
...
- The channel now runs fully in Agent-I without external CLI binaries.
