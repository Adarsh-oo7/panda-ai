#!/usr/bin/env bash
# ClawDock - Docker helpers for Agent-I
# Inspired by Simon Willison's "Running Agent-I in Docker"
# https://til.simonwillison.net/llms/Agent-I-docker
#
# Installation:
...
# Common paths to check for Agent-I
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/Agent-I"
  "${HOME}/workspace/Agent-I"
  "${HOME}/projects/Agent-I"
  "${HOME}/dev/Agent-I"
  "${HOME}/code/Agent-I"
  "${HOME}/src/Agent-I"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    param($m) $m.Value -replace 'Agent-I','Agent-I' at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      param($m) $m.Value -replace 'Agent-I','Agent-I'"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    param($m) $m.Value -replace 'Agent-I','Agent-I' not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    param($m) $m.Value -replace 'Agent-I','Agent-I'"
    param($m) $m.Value -replace 'Agent-I','Agent-I' && ./docker-setup.sh"
    echo ""
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d Agent-I-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart Agent-I-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f Agent-I-gateway
}
...
clawdock-workspace() {
  cd ~/.Agent-I/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec Agent-I-gateway \
    bash -c 'param($m) $m.Value -replace 'Agent-I','Agent-I''
}
...
clawdock-cli() {
  _clawdock_compose run --rm Agent-I-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build Agent-I-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENCLAW_GATEWAY_TOKEN=$token" Agent-I-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" Agent-I-gateway \
    bash -c './Agent-I.mjs config set gateway.remote.token "$TOKEN" && ./Agent-I.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec Agent-I-gateway \
    bash -c "./Agent-I.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart Agent-I-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm Agent-I-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec Agent-I-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'Agent-I config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Agent-I${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (Agent-I alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to Agent-I project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.Agent-I)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'Agent-I channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'Agent-I status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.Agent-I.ai${_CLR_RESET}"
  echo ""
}
