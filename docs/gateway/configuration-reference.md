...
description: "Complete field-by-field reference for ~/.Agent-I/Agent-I.json"
summary: "Complete reference for every Agent-I config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.Agent-I/Agent-I.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Agent-I uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `Agent-I doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `Agent-I doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["Agent-I-dm"] },
      guilds: {
        "123456789012345678": {
...
- Agent-I additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `Agent-I plugins install @Agent-I/mattermost`.
...
- `commands.callbackUrl` must resolve to the Agent-I gateway endpoint and be reachable from the Mattermost server.
...
Agent-I spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `Agent-I channels add` (or channel onboarding) while still on a single-account top-level channel config, Agent-I moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `Agent-I doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@Agent-I"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `Agent-I.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Agent-I auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Agent-I assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "Agent-I-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.Agent-I/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `Agent-I.json`.
noVNC observer access uses VNC auth by default and Agent-I emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `Agent-I-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from Agent-I_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `Agent-I_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `Agent-I_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `Agent-I_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        agentDir: "~/.Agent-I/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@Agent-I"] },
        sandbox: { mode: "off" },
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Agent-I starts a fresh thread session instead of inheriting parent transcript history.
...
- Files are materialized into the child workspace at `.Agent-I/attachments/<uuid>/` with a `.manifest.json`.
...
Agent-I uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.Agent-I/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `Agent-I_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Agent-I forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `Agent-I onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `Agent-I onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `Agent-I onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `Agent-I onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `Agent-I onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `Agent-I onboard --auth-choice minimax-api`.
...
- Loaded from `~/.Agent-I/extensions`, `<workspace>/.Agent-I/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `Agent-I plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or Agent-I_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `Agent-I_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `Agent-I_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
Convenience flags: `--dev` (uses `~/.Agent-I-dev` + port `19001`), `--profile <name>` (uses `~/.Agent-I-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.Agent-I/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-Agent-I-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `Agent-I_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or Agent-I_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__Agent-I__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__Agent-I__/a2ui/`
...
- Also serves A2UI at `/__Agent-I__/a2ui/`.
...
- Hostname defaults to `Agent-I`. Override with `Agent-I_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.Agent-I/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `Agent-I dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.Agent-I/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${Agent-I_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `Agent-I.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.Agent-I/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/Agent-I-vault-resolver",
...
- Legacy OAuth imports from `~/.Agent-I/credentials/oauth.json`.
...
- Default log file: `/tmp/Agent-I/Agent-I-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Agent-I.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `Agent-I_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `Agent-I doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `Agent-I.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
