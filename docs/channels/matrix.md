...
Matrix is an open, decentralized messaging protocol. Agent-I connects as a Matrix **user**
...
```bash
Agent-I plugins install @Agent-I/matrix
```

Local checkout (when running from a git repo):

```bash
Agent-I plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `Agent-I plugins install @Agent-I/matrix`
   - From a local checkout: `Agent-I plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Agent-I calls the same
     login endpoint, stores the access token in `~/.Agent-I/credentials/matrix/credentials.json`,
...
- On first connection, Agent-I requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Agent-I logs a warning.
...
`~/.Agent-I/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `Agent-I pairing list matrix`
  - `Agent-I pairing approve matrix <CODE>`
...
- On startup, Agent-I resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
