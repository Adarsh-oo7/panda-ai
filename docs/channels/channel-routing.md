...
Agent-I routes replies **back to the channel where a message came from**. The
...
Agent-I infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Agent-I still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Agent-I would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.Agent-I/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.Agent-I`):

- `~/.Agent-I/agents/<agentId>/sessions/sessions.json`
