---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Agent-I deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Agent-I cron scheduling for periodic checks, or version status checks on a machine running Agent-I (laptop, workstation, Pi, VPS).
---

# Agent-I Host Hardening

## Overview

Assess and harden the host running Agent-I, then align it to a user-defined risk tolerance without breaking access. Use Agent-I security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Agent-I changes the host firewall, SSH, or OS updates; it does not.
...
5. Agent-I gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Agent-I can access sensitive data.
...
### 2) Run Agent-I security audits (read-only)

As part of the default read-only checks, run `Agent-I security audit --deep`. Only offer alternatives if the user requests them:

1. `Agent-I security audit` (faster, non-probing)
2. `Agent-I security audit --json` (structured output)

Offer to apply Agent-I safe defaults (numbered):

1. `Agent-I security audit --fix`

Be explicit that `--fix` only tightens Agent-I defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Agent-I version/update status (read-only)

As part of the default read-only checks, run `Agent-I update status`.
...
- Credential hygiene notes (location of Agent-I creds, prefer disk encryption)
...
After Agent-I install or first hardening pass, run at least one baseline audit and version check:

- `Agent-I security audit`
- `Agent-I security audit --deep`
- `Agent-I update status`

Ongoing monitoring is recommended. Use the Agent-I cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `Agent-I cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `Agent-I update status`
...
Before creating, `Agent-I cron list` and match on exact `name`. If found, `Agent-I cron edit <id> ...`.
If not found, `Agent-I cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `Agent-I update status` (preferred for source checkouts and channels)
2. `npm view Agent-I version` (published npm version)

## Agent-I command accuracy

Use only supported commands and flags:

- `Agent-I security audit [--deep] [--fix] [--json]`
- `Agent-I status` / `Agent-I status --deep`
- `Agent-I health --json`
- `Agent-I update status`
- `Agent-I cron add|list|runs|run`

Do not invent CLI flags or imply Agent-I enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Agent-I compaction:
