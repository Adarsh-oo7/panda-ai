---
summary: "Chrome extension: let Agent-I drive your existing Chrome tab"
read_when:
...
The Agent-I Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate Agent-I-managed Chrome profile.
...
Agent-I then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Agent-I release (npm package) as static files. There is no separate “build” step.

After upgrading Agent-I:

- Re-run `Agent-I browser extension install` to refresh the installed files under your Agent-I state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Agent-I ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `Agent-I_GATEWAY_TOKEN`)
...
- Open the tab you want Agent-I to control.
...
- `ON`: attached; Agent-I can drive that tab.
...
Debugging: `Agent-I sandbox explain`
...
`Agent-I browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `Agent-I browser extension install` first to copy the extension to a stable location under your Agent-I state directory.
...
- **This is not isolated** like the dedicated Agent-I-managed profile.
