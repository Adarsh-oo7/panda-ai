...
- Jobs persist under `~/.Agent-I/cron/` so restarts don’t lose schedules.
...
Agent-I cron list
Agent-I cron run <job-id>
Agent-I cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.Agent-I/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `Agent-I cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Agent-I applies a
...
If `delivery` is omitted for isolated jobs, Agent-I defaults to `announce`.
...
- Job store: `~/.Agent-I/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.Agent-I/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Agent-I classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Agent-I archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `Agent-I cron runs` and adjust retention before logs become large
...
Agent-I cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
Agent-I cron edit <jobId> --agent ops
Agent-I cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
Agent-I cron run <jobId>
Agent-I cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
Agent-I cron edit <jobId> \
...
```bash
Agent-I system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`Agent-I system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `Agent-I_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Agent-I applies exponential retry backoff for recurring jobs after consecutive errors:
