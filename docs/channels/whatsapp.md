...
```bash
agenti channels login --channel whatsapp
```

    For a specific account:

```bash
agenti channels login --channel whatsapp --account work
```
...
```bash
agenti pairing list whatsapp
agenti pairing approve whatsapp <CODE>
```
...
<Note>
Agent-I recommends running WhatsApp on a separate number when possible. (The channel metadata and onboarding flow are optimized for that setup, but personal-number setups are also supported.)
</Note>
...
<AccordionGroup>
  <Accordion title="Dedicated number (recommended)">
    ...
    - separate WhatsApp identity for Agent-I
  ...
  <Accordion title="WhatsApp Web-only channel scope">
    The messaging platform channel is WhatsApp Web-based (`Baileys`) in current Agent-I channel architecture.
...
- if `messages.responsePrefix` is unset, self-chat replies default to `[{identity.name}]` or `[agent-i]`
...
<AccordionGroup>
  ...
  <Accordion title="Credential paths and legacy compatibility">
    - current auth path: `~/.agenti/credentials/whatsapp/<accountId>/creds.json`
    - backup file: `creds.json.bak`
    - legacy paths in `~/.Agent-I/` and `~/.agenti/credentials/` are still recognized/migrated for default-account flows
  </Accordion>

  <Accordion title="Logout behavior">
    `agenti channels logout --channel whatsapp [--account <id>]` clears WhatsApp auth state for that account.
...
<AccordionGroup>
  ...
  <Accordion title="Group messages unexpectedly ignored">
    ...
    - duplicate keys in `Agent-I.json` (JSON5): later entries override earlier ones, so keep a single `groupPolicy` per scope
