...
- `/usage cost` in chats: local cost summary aggregated from Agent-I session logs.
- CLI: `Agent-I status --usage` prints a full per-provider breakdown.
- CLI: `Agent-I channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
