...
```bash
Agent-I onboard --auth-choice moonshot-api-key
```

Kimi Coding:

```bash
Agent-I onboard --auth-choice kimi-code-api-key
```
...
Agent-I also maps runtime `/think` levels for Moonshot:

- `/think off` -> `thinking.type=disabled`
- any non-off thinking level -> `thinking.type=enabled`

When Moonshot thinking is enabled, `tool_choice` must be `auto` or `none`. Agent-I normalizes incompatible `tool_choice` values to `auto` for compatibility.
