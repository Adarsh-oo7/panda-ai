...
> Agent-I is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
Agent-I security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `Agent-I security audit`
...
Agent-I security audit --deep
Agent-I security audit --fix
Agent-I security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

Agent-I is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
Agent-I assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.Agent-I`, including `Agent-I.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- Agent-I can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want Agent-I, use one VPS/host per user.
...
Agent-I is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/Agent-I/Agent-I/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, Agent-I also attempts a best-effort live Gateway probe.
...
| -------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full Agent-I state                                 | filesystem perms on `~/.Agent-I`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.Agent-I/Agent-I.json`                                                   | yes      |
...
`Agent-I security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`Agent-I security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in Agent-I config
schema:
...
gateway:
  ...
  auth:
    mode: password
    password: ${Agent-I_GATEWAY_PASSWORD}
```
...
- Agent-I gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from Agent-I responses.
...
Agent-I stores session transcripts on disk under `~/.Agent-I/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.Agent-I` (see the audit section below). If you need
...
Agent-I can refresh the skills list mid-session:
...
- If you install plugins from npm (`Agent-I plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.Agent-I/extensions/<pluginId>/` (or `$Agent-I_STATE_DIR/extensions/<pluginId>/`).
  - Agent-I uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
Agent-I pairing list <channel>
Agent-I pairing approve <channel> <code>
```
...
By default, Agent-I routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Agent-I has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.Agent-I/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.Agent-I or your logs.”

## Unsafe external content bypass flags

Agent-I includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.Agent-I/Agent-I.json`: `600` (user read/write only)
- `~/.Agent-I`: `700` (user only)

`Agent-I doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `Agent-I_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__Agent-I__/canvas/` and `/__Agent-I__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_Agent-I-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `Agent-I_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `Agent-I doctor --generate-gateway-token`.
...
paths, set `Agent-I_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `Agent-I_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `Agent-I_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Agent-I
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Agent-I verifies the identity by resolving the
...
- Agent-I will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.Agent-I/` (or `$Agent-I_STATE_DIR/`) may contain secrets or private data:

- `Agent-I.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `Agent-I status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@Agent-I", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.Agent-I`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.Agent-I/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `Agent-I` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Agent-I clients can connect.
- Disable browser proxy routing when you don’t need it (`gateway.nodes.browser.mode="off"`).
- Chrome extension relay mode is **not** “safer”; it can take over your existing Chrome tabs. Assume it can act as you in whatever that tab/profile can reach.

### Browser SSRF policy (trusted-network default)

Agent-I’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Agent-I limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `Agent-I gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `Agent-I_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/Agent-I/Agent-I-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.Agent-I/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `Agent-I security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Agent-I version
...
Found a vulnerability in Agent-I? Please report responsibly:

1. Email: [security@Agent-I.ai](mailto:security@Agent-I.ai)
