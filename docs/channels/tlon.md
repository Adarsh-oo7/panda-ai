...
Tlon is a decentralized messenger built on Urbit. Agent-I connects to your Urbit ship and can
...
```bash
Agent-I plugins install @Agent-I/tlon
```

Local checkout (when running from a git repo):

```bash
Agent-I plugins install ./extensions/tlon
```
...
By default, Agent-I blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `Agent-I message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Agent-I replies in-thread.
