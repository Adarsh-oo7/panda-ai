...
你可以在 `~/.Agent-I/Agent-I.json` 中覆盖此设置：
...
所有日志配置都在 `~/.Agent-I/Agent-I.json` 的 `logging` 下。
...
- Agent-I 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `Agent-I plugins enable diagnostics-otel` 启用插件。
...
- `Agent-I.tokens`（计数器，属性：`Agent-I.token`、`Agent-I.channel`、`Agent-I.provider`、`Agent-I.model`）
- `Agent-I.cost.usd`（计数器，属性：`Agent-I.channel`、`Agent-I.provider`、`Agent-I.model`）
- `Agent-I.run.duration_ms`（直方图，属性：`Agent-I.channel`、`Agent-I.provider`、`Agent-I.model`）
- `Agent-I.context.tokens`（直方图，属性：`Agent-I.context`、`Agent-I.channel`、`Agent-I.provider`、`Agent-I.model`）

消息流：

- `Agent-I.webhook.received`（计数器，属性：`Agent-I.channel`、`Agent-I.webhook`）
- `Agent-I.webhook.error`（计数器，属性：`Agent-I.channel`、`Agent-I.webhook`）
- `Agent-I.webhook.duration_ms`（直方图，属性：`Agent-I.channel`、`Agent-I.webhook`）
- `Agent-I.message.queued`（计数器，属性：`Agent-I.channel`、`Agent-I.source`）
- `Agent-I.message.processed`（计数器，属性：`Agent-I.channel`、`Agent-I.outcome`）
- `Agent-I.message.duration_ms`（直方图，属性：`Agent-I.channel`、`Agent-I.outcome`）

队列 + 会话：

- `Agent-I.queue.lane.enqueue`（计数器，属性：`Agent-I.lane`）
- `Agent-I.queue.lane.dequeue`（计数器，属性：`Agent-I.lane`）
- `Agent-I.queue.depth`（直方图，属性：`Agent-I.lane` 或 `Agent-I.channel=heartbeat`）
- `Agent-I.queue.wait_ms`（直方图，属性：`Agent-I.lane`）
- `Agent-I.session.state`（计数器，属性：`Agent-I.state`、`Agent-I.reason`）
- `Agent-I.session.stuck`（计数器，属性：`Agent-I.state`）
- `Agent-I.session.stuck_age_ms`（直方图，属性：`Agent-I.state`）
- `Agent-I.run.attempt`（计数器，属性：`Agent-I.attempt`）
...
- `Agent-I.webhook.error`
  - `Agent-I.channel`、`Agent-I.webhook`、`Agent-I.chatId`、`Agent-I.error`
- `Agent-I.message.processed`
  - `Agent-I.channel`、`Agent-I.outcome`、`Agent-I.chatId`、`Agent-I.messageId`、`Agent-I.sessionKey`、`Agent-I.sessionId`、`Agent-I.reason`
- `Agent-I.session.stuck`
  - `Agent-I.state`、`Agent-I.ageMs`、`Agent-I.queueDepth`、`Agent-I.sessionKey`、`Agent-I.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `Agent-I doctor`。
