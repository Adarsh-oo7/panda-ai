...
This plan defines how Agent-I should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Agent-I core owns ACP control plane concerns
...
Agent-I should not reimplement ACP transport internals in core.
Agent-I should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Agent-I, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Agent-I state dir
...
- ACP event stream is normalized to Agent-I reply chunks
...
For the first production backend (`extensions/acpx`), Agent-I and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Agent-I `sessionKey`)
...
- Agent-I consumes ndjson events from `acpx --format json --json-strict`
