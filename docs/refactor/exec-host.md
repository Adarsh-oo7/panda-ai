...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.Agent-I/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.Agent-I/node.json` (node id + pairing token).
...
Path: `~/.Agent-I/exec-approvals.json`
...
- Unix socket at `~/.Agent-I/exec-approvals.sock` (0600).
