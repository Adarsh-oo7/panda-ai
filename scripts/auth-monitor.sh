...
#
# Suggested cron: */30 * * * * /home/admin/Agent-I/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Agent-I notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., Agent-I-alerts)
...
STATE_FILE="$HOME/.Agent-I/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Agent-I if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use Agent-I
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            param($m) $m.Value -replace 'Agent-I','Agent-I' to $NOTIFY_PHONE..."
            Agent-I send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Agent-I is down. Run: ssh l36 '~/Agent-I/scripts/mobile-reauth.sh'" "urgent"
    exit 1
