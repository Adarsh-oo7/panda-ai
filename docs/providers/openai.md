---
summary: "Use OpenAI via API keys or Codex subscription in Agent-I"
read_when:
  - You want to use OpenAI models in Agent-I
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Agent-I.
...
```bash
Agent-I onboard --auth-choice openai-api-key
# or non-interactive
Agent-I onboard --openai-api-key "$OPENAI_API_KEY"
```
...
Agent-I onboard --auth-choice openai-codex

# Or run OAuth directly
Agent-I models auth login --provider openai-codex
```
...
Agent-I uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Agent-I also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Agent-I enables it by default for
...
`baseUrl` on `api.openai.com`), Agent-I now auto-enables OpenAI server-side
