...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "Agent-I")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || param($m) $m.Value -replace 'Agent-I','Agent-I'")
...
DMG_TEMP="$(mktemp -d /tmp/Agent-I-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/Agent-I/Resources/Agent-I.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/Agent-I-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/Agent-I-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/Agent-I-dmg-limits.txt
