---
summary: "How Agent-I sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Agent-I can run **tools inside Docker containers** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`Agent-I-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; Agent-I emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
- `"none"` (default): tools see a sandbox workspace under `~/.Agent-I/sandboxes`.
- `"ro"`: mounts the agent workspace read-only at `/agent` (disables `write`/`edit`/`apply_patch`).
- `"rw"`: mounts the agent workspace read/write at `/workspace`.

Inbound media is copied into the active sandbox workspace (`media/inbound/*`).
Skills note: the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`,
Agent-I mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- Agent-I blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
Default image: `Agent-I-sandbox:bookworm-slim`
...
`Agent-I-sandbox-common:bookworm-slim`.
...
- The three graphics hardening flags (`--disable-3d-apis`,
  ...
  when containers lack GPU support. Set `Agent-I_BROWSER_DISABLE_GRAPHICS_FLAGS=0`
...
- `--disable-extensions` is enabled by default and can be disabled with
  `Agent-I_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
- `--renderer-process-limit=2` is controlled by
  `Agent-I_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
Set `Agent-I_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can
override socket location with `Agent-I_DOCKER_SOCKET`. Full setup and env
...
- Use `Agent-I sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
