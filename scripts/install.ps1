# Agent-I Installer for Windows (PowerShell)
# Usage: iwr -useb https://Agent-I.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://Agent-I.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\Agent-I",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 Agent-I Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Agent-I.$NC" -Level info
    Write-Host ""
}
...
function Install-Agent-INpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing Agent-I (Agent-I@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g Agent-I@$Version --no-fund --no-audit 2>&1
        Write-Host "Agent-I installed" -Level success
        return $true
    } catch {
...
function Install-Agent-IGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Agent-I from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/Agent-I/Agent-I.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\Agent-I\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\Agent-I.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Agent-I from git to $GitDir" -Level info
        } else {
            Install-Agent-IGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Agent-I via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-Agent-INpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'Agent-I onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 Agent-I installed successfully!" -Level success
}
