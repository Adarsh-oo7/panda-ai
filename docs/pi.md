...
summary: "Architecture of Agent-I's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Agent-I
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Agent-I integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Agent-I uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Agent-I directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Agent-I's local TUI mode)                                             |
...
├── pi-tools.ts                    # createAgent-ICodingTools()
...
├── Agent-I-tools.ts              # Agent-I-specific tools
...
Image injection is prompt-local: Agent-I loads image refs from the current prompt and
...
2. **Custom Replacements**: Agent-I replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Agent-I Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Agent-I's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Agent-I CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Agent-I wraps this with `guardSessionManager()` for tool result safety.
...
Agent-I maintains an auth profile store with multiple API keys per provider:
...
Agent-I loads custom pi extensions for specialized behavior:
...
Agent-I also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Agent-I Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Agent-I tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.Agent-I/agents/<agentId>/sessions/` (or `$Agent-I_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `Agent-I_LIVE_TEST=1`)
