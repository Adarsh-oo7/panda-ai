...
```bash
Agent-I tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `Agent-I tui --deliver`
...
- Local shell commands receive `Agent-I_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `Agent-I logs --follow`.
- Confirm the agent can run: `Agent-I status` and `Agent-I models status`.
...
- No agents in picker: check `Agent-I agents list` and your routing config.
