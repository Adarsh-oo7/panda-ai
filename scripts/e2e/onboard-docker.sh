...
IMAGE_NAME="Agent-I-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/Agent-I-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/Agent-I-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/Agent-I-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/Agent-I-e2e-$1.XXXXXX"
  }
  ...
  set_isolated_OPENCLAW_env() {
    ...
    export OPENCLAW_STATE_DIR="$home_dir/.Agent-I"
    export OPENCLAW_CONFIG_PATH="$OPENCLAW_STATE_DIR/Agent-I.json"
