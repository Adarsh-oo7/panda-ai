...
```
    ...
    Telegram does **not** use `Agent-I channels login telegram`; configure token in config/env, then start gateway.
...
Agent-I pairing approve telegram <CODE>
```
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `Agent-I doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `Agent-I doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `Agent-I logs --follow`
...
- DM messages can carry `message_thread_id`; Agent-I routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (native drafts + message edits)">
    Agent-I can stream partial replies in real time:
    ...
    - DM: Agent-I updates the draft in place (no extra preview message)
    - group/topic: Agent-I keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), Agent-I falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, Agent-I skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, Agent-I automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Agent-I retries as plain text.
...
```
    ...
    - `~/.Agent-I/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Agent-I enqueues system events like:
...
```bash
Agent-I message send --channel telegram --target 123456789 --message "hi"
Agent-I message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `Agent-I channels status` warns when config expects unmentioned group messages.
    - `Agent-I channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `Agent-I logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, Agent-I now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `Agent-I_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `Agent-I_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `Agent-I_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `Agent-I doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `Agent-I doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, Agent-I falls back to the first normalized account ID and `Agent-I doctor` warns.
