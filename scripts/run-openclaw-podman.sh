#!/usr/bin/env bash
# Rootless Agent-I in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./setup-podman.sh
# Then:
#   ./scripts/run-Agent-I-podman.sh launch           # Start gateway
#   ./scripts/run-Agent-I-podman.sh launch setup      # Onboarding wizard
#
# As the Agent-I user (no repo needed):
...
OPENCLAW_USER="${OPENCLAW_PODMAN_USER:-Agent-I}"
...
LAUNCH_SCRIPT="$OPENCLAW_HOME/run-Agent-I-podman.sh"
...
# --- Step 2: launch (from repo: re-exec as Agent-I in safe cwd; from Agent-I home: run container) ---
if [[ "${1:-}" == "launch" ]]; then
  shift
  if [[ -n "${OPENCLAW_UID:-}" && "$(id -u)" -ne "$OPENCLAW_UID" ]]; then
    # Exec as Agent-I with cwd=/tmp so a nologin user never inherits an invalid cwd.
    exec sudo -u "$OPENCLAW_USER" env HOME="$OPENCLAW_HOME" PATH="$PATH" TERM="${TERM:-}" \
      bash -c 'cd /tmp && exec '"$LAUNCH_SCRIPT"' "$@"' _ "$@"
  fi
  # Already Agent-I; fall through to container run (with remaining args, e.g. "setup")
fi

# --- Container run (script in Agent-I home, run as Agent-I) ---
EFFECTIVE_HOME="${HOME:-}"
...
CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.Agent-I}"
ENV_FILE="${OPENCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${OPENCLAW_PODMAN_CONTAINER:-Agent-I}"
OPENCLAW_IMAGE="${OPENCLAW_PODMAN_IMAGE:-Agent-I:local}"
...
# Safe cwd for podman (Agent-I is nologin; avoid inherited cwd from sudo)
...
PY
  ...
  param($m) $m.Value -replace 'Agent-I','Agent-I'_GATEWAY_TOKEN." >&2
  exit 1
}
...
if [[ -z "${OPENCLAW_GATEWAY_TOKEN:-}" ]]; then
  ...
  param($m) $m.Value -replace 'Agent-I','Agent-I'_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

# The gateway refuses to start unless gateway.mode=local is set in config.
# Keep this minimal; users can run the wizard later to configure channels/providers.
CONFIG_JSON="$CONFIG_DIR/Agent-I.json"
...
case "$PODMAN_USERNS" in
  ...
  *)
    param($m) $m.Value -replace 'Agent-I','Agent-I'_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  param($m) $m.Value -replace 'Agent-I','Agent-I'_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi
...
if [[ "$RUN_SETUP" == true ]]; then
  exec podman run --pull="$PODMAN_PULL" --rm -it \
    ...
    -v "$CONFIG_DIR:/home/node/.Agent-I:rw" \
    -v "$WORKSPACE_DIR:/home/node/.Agent-I/workspace:rw" \
    "${ENV_FILE_ARGS[@]}" \
    "$OPENCLAW_IMAGE" \
