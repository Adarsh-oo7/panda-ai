...
read_when:
  - You need detailed behavior for Agent-I onboard
...
This page is the full reference for `Agent-I onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.Agent-I/Agent-I.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `Agent-I doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `Agent-I pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `Agent-I health`.
    - `Agent-I status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.Agent-I/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.Agent-I/agents/<agentId>/agent/auth-profiles.json`
...
`~/.Agent-I/credentials/oauth.json` (or `$Agent-I_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`Agent-I onboard`](/cli/onboard)
