...
APP_BUNDLE="${1:-dist/Agent-I.app}"
IDENTITY="${SIGN_IDENTITY:-}"
TIMESTAMP_MODE="${CODESIGN_TIMESTAMP:-auto}"
DISABLE_LIBRARY_VALIDATION="${DISABLE_LIBRARY_VALIDATION:-0}"
SKIP_TEAM_ID_CHECK="${SKIP_TEAM_ID_CHECK:-0}"
ENT_TMP_BASE=$(mktemp -t Agent-I-entitlements-base.XXXXXX)
ENT_TMP_APP_BASE=$(mktemp -t Agent-I-entitlements-app-base.XXXXXX)
ENT_TMP_RUNTIME=$(mktemp -t Agent-I-entitlements-runtime.XXXXXX)
...
# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/Agent-I" ]; then
  param($m) $m.Value -replace 'Agent-I','Agent-I'" "$APP_ENTITLEMENTS"
fi
