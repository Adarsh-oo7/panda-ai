...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `Agent-I sandbox explain`.
...
Legacy `agent.*` configs are migrated by `Agent-I doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=Agent-I-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${Agent-I_STATE_DIR:-$HOME/.Agent-I}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
