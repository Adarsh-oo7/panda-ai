...
This document outlines the design for integrating "Kilo Gateway" as a first-class provider in Agent-I, modeled after the existing OpenRouter implementation. Kilo Gateway uses an OpenAI-compatible completions API with a different base URL.
...
export function applyKilocodeProviderConfig(cfg: Agent-IConfig): Agent-IConfig {
...
export function applyKilocodeConfig(cfg: Agent-IConfig): Agent-IConfig {
