...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Agent-I
...
Running Agent-I via Node with `tsx` fails at startup with:

```
[Agent-I] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `Agent-I status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch Agent-I.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node Agent-I.mjs status` works on Node 25.
