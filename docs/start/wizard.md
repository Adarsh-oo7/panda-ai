...
The onboarding wizard is the **recommended** way to set up Agent-I on macOS,
...
`Agent-I dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
(`web_fetch` works without a key). Easiest path: `Agent-I configure --section web`
...
2. **Workspace** — Location for agent files (default `~/.Agent-I/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `Agent-I doctor` first.
</Note>
...
Use `Agent-I agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.Agent-I/workspace-<agentId>`.
...
- CLI command reference: [`Agent-I onboard`](/cli/onboard)
