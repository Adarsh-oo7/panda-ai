...
- **gateway host** → `Agent-I` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.Agent-I/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.Agent-I/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `Agent-I approvals allowlist ...`).
- `Agent-I security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `Agent-I doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.Agent-I/exec-approvals.json` directly.

CLI: `Agent-I approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
