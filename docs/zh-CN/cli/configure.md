...
summary: "`Agent-I configure` 的 CLI 参考（交互式配置提示）"
title: configure
x-i18n:
...
提示：不带子命令的 `Agent-I config` 会打开相同的向导。使用 `Agent-I config get|set|unset` 进行非交互式编辑。
...
```bash
Agent-I configure
Agent-I configure --section models --section channels
```
