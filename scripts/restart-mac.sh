#!/usr/bin/env bash
# Reset Agent-I like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${OPENCLAW_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Agent-I.app/Contents/MacOS/Agent-I"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Agent-I"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Agent-I"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Agent-I"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.Agent-I.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/Agent-I-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${OPENCLAW_RESTART_LOG:-/tmp/Agent-I-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${OPENCLAW_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.Agent-I/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node Agent-I.mjs daemon install --force --runtime node"
      log "  node Agent-I.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.Agent-I/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.Agent-I.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Agent-I instances"
kill_all_Agent-I
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Agent-I"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Agent-I.app" ]]; then
    APP_BUNDLE="/Applications/Agent-I.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Agent-I.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Agent-I.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Agent-I.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set OPENCLAW_APP_BUNDLE to your installed Agent-I.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node Agent-I.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node Agent-I.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".Agent-I", "Agent-I.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: Agent-I is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.Agent-I.gateway.plist' | head -n 40 || true"
fi
