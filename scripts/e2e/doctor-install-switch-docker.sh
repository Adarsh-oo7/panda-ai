...
IMAGE_NAME="Agent-I-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/Agent-I-bin:$PATH"
  mkdir -p /tmp/Agent-I-bin

  cat > /tmp/Agent-I-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/Agent-I-bin/systemctl

  cat > /tmp/Agent-I-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/Agent-I-bin/loginctl
  ...
  npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz"
	  ...
	  npm_bin="/tmp/npm-prefix/bin/Agent-I"
	  npm_root="/tmp/npm-prefix/lib/node_modules/Agent-I"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/Agent-I-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/Agent-I-gateway.service"
    if [ ! -f "$unit_path" ]; then
