...
- During heartbeat runs, Agent-I treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `Agent-I sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Agent-I skips the heartbeat run entirely (no model call).
...
```bash
Agent-I system event --text "Check for urgent follow-ups" --mode now
```
