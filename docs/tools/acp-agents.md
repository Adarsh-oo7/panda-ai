...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Agent-I run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask Agent-I in plain language to "run this in Codex" or "start Claude Code in a thread", Agent-I should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Agent-I-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Agent-I native sub-agent runtime  |
...
- Agent-I binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Agent-I returns a clear unsupported/unavailable message.
...
- If `agentId` is omitted, Agent-I uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Agent-I may default to persistent behavior per runtime path
...
ACP sessions currently run on the host runtime, not inside the Agent-I sandbox.
...
If no target resolves, Agent-I returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Agent-I returns a clear unsupported-control error.
...
When Agent-I uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Agent-I `agentId` path).
...
```bash
Agent-I plugins install acpx
Agent-I config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
Agent-I plugins install ./extensions/acpx
```
...
By default, the acpx plugin (published as `@Agent-I/acpx`) uses the plugin-local pinned binary:
...
- Relative paths resolve from Agent-I workspace directory.
...
- Agent-I startup remains non-blocking while the backend health check runs.
...
```bash
Agent-I config set plugins.entries.acpx.config.permissionMode approve-all
Agent-I config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** Agent-I currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
