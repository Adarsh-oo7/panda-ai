...
// Order: .agenti (new canonical) → .openclaw (previous) → .clawdbot/.moldbot/.moltbot (older)
const LEGACY_STATE_DIRNAMES = [".openclaw", ".clawdbot", ".moldbot", ".moltbot"] as const;
const NEW_STATE_DIRNAME = ".agenti";
...
export function migrateStateDirIfNeeded(homedir: () => string = () => os.homedir()): void {
  ...
  try {
    fs.cpSync(legacySource, newDir, { recursive: true });
  } catch {
    // Migration failure is non-fatal; fall back to using the legacy dir.
  }
}
