...
let openclawRoot: typeof import("./agenti-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    openclawRoot = await import("./agenti-root.js");
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    await expect(resolveControlUiDistIndexPath(path.join(root, "agenti.mjs"))).resolves.toBe(
