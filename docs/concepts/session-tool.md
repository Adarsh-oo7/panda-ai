...
- When given a `sessionId`, Agent-I resolves it to the corresponding session key (missing ids error).
...
- After the primary run completes, Agent-I runs a **reply-back loop**:
  - Round 2+ alternates between requester and target agents.
  - Reply exactly `REPLY_SKIP` to stop the ping‑pong.
  - Max turns is `session.agentToAgent.maxPingPongTurns` (0–5, default 5).
- Once the loop ends, Agent-I runs the **agent‑to‑agent announce step** (target agent only):
...
- `attachments?` (optional array of inline files; subagent runtime only, ACP rejects). Each entry: `{ name, content, encoding?: "utf8" | "base64", mimeType? }`. Files are materialized into the child workspace at `.Agent-I/attachments/<uuid>/`. Returns a receipt with sha256 per file.
...
- After completion, Agent-I runs a sub-agent **announce step** and posts the result to the requester chat channel.
...
- When a session is sandboxed and `sessionToolsVisibility="spawned"`, Agent-I clamps visibility to `tree` even if you set `tools.sessions.visibility="all"`.
