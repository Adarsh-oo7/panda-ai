...
import { migrateStateDirIfNeeded } from "../config/paths.js";
...
export async function runCli(argv: string[] = process.argv) {
  ...
  normalizeEnv();
  // Auto-migrate state dir from ~/.openclaw (or older names) to ~/.agenti on first run.
  migrateStateDirIfNeeded();
  ...
  process.on("uncaughtException", (error) => {
    console.error("[agent-i] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
