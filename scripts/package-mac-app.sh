...
# Build and bundle Agent-I into a minimal .app we can open.
# Outputs to dist/Agent-I.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Agent-I.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Agent-I"
BUNDLE_ID="${BUNDLE_ID:-ai.Agent-I.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/Agent-I/Agent-I/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Agent-I/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :Agent-IBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :Agent-IGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Agent-I"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Agent-I"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Agent-I"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Agent-I" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Agent-I/Resources/Agent-I.icns" "$APP_ROOT/Contents/Resources/Agent-I.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Agent-I/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
param($m) $m.Value -replace 'Agent-I','Agent-I'Kit resources"
Agent-IKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/Agent-IKit_Agent-IKit.bundle"
if [ -d "$Agent-IKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/Agent-IKit_Agent-IKit.bundle"
  cp -R "$Agent-IKIT_BUNDLE" "$APP_ROOT/Contents/Resources/Agent-IKit_Agent-IKit.bundle"
else
  param($m) $m.Value -replace 'Agent-I','Agent-I'KIT_BUNDLE (continuing)" >&2
fi
...
killall -q Agent-I 2>/dev/null || true
