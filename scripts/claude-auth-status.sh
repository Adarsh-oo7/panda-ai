...
# Checks both Claude Code and Agent-I auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
OPENCLAW_AUTH="$HOME/.Agent-I/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    Agent-I models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    jq -n \
        ...
        '{
            ...
            Agent-I: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        ...
        param($m) $m.Value -replace 'Agent-I','Agent-I'_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$OPENCLAW_status" == EXPIRING* ]]; then
        param($m) $m.Value -replace 'Agent-I','Agent-I'_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
param($m) $m.Value -replace 'Agent-I','Agent-I'/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    param($m) $m.Value -replace 'Agent-I','Agent-I' doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active Agent-I >/dev/null 2>&1; then
    echo -e "Agent-I service: ${GREEN}running${NC}"
else
    echo -e "Agent-I service: ${RED}NOT running${NC}"
fi
