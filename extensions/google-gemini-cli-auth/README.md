# Google Gemini CLI Auth (Agent-I plugin)
...
```bash
Agent-I plugins enable google-gemini-cli-auth
```
...
```bash
Agent-I models auth login --provider google-gemini-cli --set-default
```
...
- `Agent-I_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `Agent-I_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
