---
summary: "CLI reference for `Agent-I security` (audit and fix common security footguns)"
read_when:
...
It also emits `security.trust_model.multi_user_heuristic` when config suggests likely shared-user ingress (for example open DM/group policy, configured group targets, or wildcard sender rules), and reminds you that Agent-I is a personal-assistant trust model by default.
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `Agent-I.browserConfigEpoch`) and recommends `Agent-I sandbox recreate --browser --all`.
...
```bash
Agent-I security audit --json | jq '.summary'
Agent-I security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
Agent-I security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
