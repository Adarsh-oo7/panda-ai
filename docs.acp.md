...
This document describes how the Agent-I ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`Agent-I acp` exposes an ACP agent over stdio and forwards prompts to a running
Agent-I Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `Agent-I acp` and speaks ACP messages over stdio.
...
`Agent-I acp` resolves the Gateway URL and auth from CLI flags or config:
