...
```bash
Agent-I plugins install @Agent-I/msteams
```

Local checkout (when running from a git repo):

```bash
Agent-I plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Agent-I will offer the local install path automatically.
...
3. Configure Agent-I with those credentials.
...
- Talk to Agent-I via Teams DMs, group chats, or channels.
...
- On startup, Agent-I resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.Agent-I/Agent-I.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Agent-I, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                   |
   | ------------------ | ------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `Agent-I-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `Agent-I plugins install @Agent-I/msteams`
   - From a local checkout: `Agent-I plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Agent-I in Teams", "full": "Agent-I in Teams" },
...
Agent-I handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Agent-I only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/Agent-IShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Agent-I sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `Agent-I message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.Agent-I/msteams-polls.json`.
...
```bash
Agent-I message send --channel msteams \
...
Agent-I message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
Agent-I message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
Agent-I message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
Agent-I message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
