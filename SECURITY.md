...
If you believe you've found a security issue in Agent-I, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [Agent-I/Agent-I](https://github.com/Agent-I/Agent-I)
- **macOS desktop app** — [Agent-I/Agent-I](https://github.com/Agent-I/Agent-I) (apps/macos)
- **iOS app** — [Agent-I/Agent-I](https://github.com/Agent-I/Agent-I) (apps/ios)
- **Android app** — [Agent-I/Agent-I](https://github.com/Agent-I/Agent-I) (apps/android)
- **ClawHub** — [Agent-I/clawhub](https://github.com/Agent-I/clawhub)
- **Trust and threat model** — [Agent-I/trust](https://github.com/Agent-I/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@Agent-I.ai](mailto:security@Agent-I.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.Agent-I.ai).
...
- Tested version details (Agent-I version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to Agent-I's documented trust boundaries.
- For exposed-secret reports: proof the credential is Agent-I-owned (or grants access to Agent-I-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at Agent-I. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

Agent-I is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
Agent-I does **not** model one gateway as a multi-tenant, adversarial user boundary.

- Authenticated Gateway callers are treated as trusted operators for that gateway instance.
- Session identifiers (`sessionKey`, session IDs, labels) are routing controls, not per-user authorization boundaries.
- If one operator can view data from another operator on the same gateway, that is expected in this trust model.
- Agent-I can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Agent-I, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Agent-I's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of Agent-I's trusted computing base for a gateway.
...
- Using Agent-I in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.Agent-I`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not Agent-I-owned and not granting access to Agent-I-operated infrastructure/services) without demonstrated Agent-I impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

Agent-I security guidance assumes:

- The host where Agent-I runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.Agent-I` state/config (including `Agent-I.json`) is effectively a trusted operator.
...
Agent-I's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Agent-I separates routing from execution, but both remain inside the same operator trust boundary:
...
- Plugins can execute with the same OS privileges as the Agent-I process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

Agent-I uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/Agent-I` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/Agent-I` (or `Agent-I-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Agent-I-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Agent-I temp helpers (`resolvePreferredAgent-ITmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-Agent-I-dir.ts`
...
For threat model + hardening guidance (including `Agent-I security audit --deep` and `--fix`), see:

- `https://docs.Agent-I.ai/gateway/security`
...
Agent-I's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `Agent-I gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - Agent-I keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `Agent-I security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__Agent-I__/canvas/`, `/__Agent-I__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__Agent-I__/canvas/`, `/__Agent-I__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

Agent-I requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running Agent-I in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v Agent-I-data:/app/data \
  Agent-I/Agent-I:latest
```
