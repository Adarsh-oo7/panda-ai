...
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "agenti-root");
...
describe("resolveOpenClawPackageRoot", () => {
  let resolveOpenClawPackageRoot: typeof import("./agenti-root.js").resolveOpenClawPackageRoot;
  let resolveOpenClawPackageRootSync: typeof import("./agenti-root.js").resolveOpenClawPackageRootSync;

  beforeAll(async () => {
    ({ resolveOpenClawPackageRoot, resolveOpenClawPackageRootSync } =
      await import("./agenti-root.js"));
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "agenti.mjs")));
