...
read_when:
  - Running Agent-I behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Agent-I
...
- You run Agent-I behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Agent-I checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Agent-I extracts the user identity from the configured header
...
- Agent-I can stay on loopback HTTP behind the proxy.
...
If Agent-I itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://Agent-I.example.com
    to: http://Agent-I-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`Agent-I security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Agent-I config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `Agent-I security audit` and review findings
