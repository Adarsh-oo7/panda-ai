...
`Agent-I.invoke --each`) without adding new Agent-I code per workflow.
...
This extension depends on Agent-I internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Agent-I extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.Agent-I/extensions` as a standalone plugin directory.
