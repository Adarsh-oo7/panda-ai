...
Agent-I supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Agent-I runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Agent-I validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Agent-I QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/Agent-I"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `Agent-I.json` refs)
...
When reload-time activation fails after a healthy state, Agent-I enters degraded secrets state.
...
Credential-sensitive command paths that opt in (for example `Agent-I memory` remote-memory paths and `Agent-I qr --remote`) can resolve supported SecretRefs via gateway snapshot RPC.
...
- Snapshot refresh after backend secret rotation is handled by `Agent-I secrets reload`.
...
```bash
Agent-I secrets audit --check
Agent-I secrets configure
Agent-I secrets audit --check
```
...
- plaintext values at rest (`Agent-I.json`, `auth-profiles.json`, `.env`)
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `Agent-I.json` refs)
...
- lets you select supported secret-bearing fields in `Agent-I.json` plus `auth-profiles.json` for one agent scope
...
- `Agent-I secrets configure --providers-only`
- `Agent-I secrets configure --skip-provider-setup`
- `Agent-I secrets configure --agent <id>`
...
```bash
Agent-I secrets apply --from /tmp/Agent-I-secrets-plan.json
Agent-I secrets apply --from /tmp/Agent-I-secrets-plan.json --dry-run
```
...
Agent-I intentionally does not write rollback backups containing historical plaintext secret values.
