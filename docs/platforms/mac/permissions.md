...
- Same path: run the app from a fixed location (for Agent-I, `dist/Agent-I.app`).
...
```bash
sudo tccutil reset Accessibility ai.Agent-I.mac
sudo tccutil reset ScreenCapture ai.Agent-I.mac
...
Workaround: move files into the Agent-I workspace (`~/.Agent-I/workspace`) if you want to avoid per-folder grants.
