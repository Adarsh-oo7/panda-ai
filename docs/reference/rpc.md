...
Agent-I integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Agent-I owns lifecycle when `channels.signal.autoStart=true`.
...
- Agent-I spawns `imsg rpc` as a child process (legacy iMessage integration).
