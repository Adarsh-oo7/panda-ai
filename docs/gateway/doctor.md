...
`Agent-I doctor` is the repair + migration tool for Agent-I. It fixes stale
...
```bash
Agent-I doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
Agent-I doctor --repair --force
```
...
- Extra workspace dir detection (`~/Agent-I`).
...
you to run `Agent-I doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.Agent-I/Agent-I.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.Agent-I/sessions/` to `~/.Agent-I/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.Agent-I/agent/` to `~/.Agent-I/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.Agent-I/credentials/*.json` (except `oauth.json`)
  - to `~/.Agent-I/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `Agent-I doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.Agent-I` folders exist across
  home directories or when `Agent-I_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.Agent-I/Agent-I.json` is
...
offers to remove them and install the Agent-I service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Agent-I gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `Agent-I doctor --generate-gateway-token` to force token
...
- `Agent-I doctor` prompts before rewriting supervisor config.
- `Agent-I doctor --yes` accepts the default repair prompts.
- `Agent-I doctor --repair` applies recommended fixes without prompts.
- `Agent-I doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `Agent-I gateway install --force`.
