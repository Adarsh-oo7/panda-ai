...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the Agent-I (Panda AI) gateway (Agent-I.app menubar, or \`${formatCliCommand("agenti gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the Agent-I browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the Agent-I browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
