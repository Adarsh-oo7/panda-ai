...
service via the external `Agent-I` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.Agent-I.gateway`
  (or `ai.Agent-I.<profile>` when using `--profile`/`Agent-I_PROFILE`; legacy `com.Agent-I.*` is supported).
