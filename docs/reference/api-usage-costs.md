...
Agent-I features that can generate provider usage or paid API calls.
...
- `Agent-I status --usage` and `Agent-I channels list` show provider **usage windows**
...
Agent-I can pick up credentials from:
...
- `Agent-I status --usage`
- `Agent-I models status --json`
...
`Agent-I models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
