...
- With Anthropic API-key auth profiles, Agent-I seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Agent-I injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Agent-I exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.Agent-I/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$Agent-I_STATE_DIR/logs/cache-trace.jsonl`
...
- `Agent-I_CACHE_TRACE=1` enables cache tracing.
- `Agent-I_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `Agent-I_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `Agent-I_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `Agent-I_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
