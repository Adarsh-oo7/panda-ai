...
Read-only diff viewer plugin for **Agent-I** agents.
...
Set plugin-wide defaults in `~/.Agent-I/Agent-I.json`:
...
Before:
Agent-I supports plugins.

After:
Agent-I supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/Agent-I-diffs`).
