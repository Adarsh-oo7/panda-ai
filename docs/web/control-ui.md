...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/Agent-I`)
...
If the page fails to load, start the Gateway first: `Agent-I gateway`.
...
you revoke it with `Agent-I devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.Agent-I/Agent-I.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop Agent-I`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Agent-I
...
Agent-I **blocks** Control UI connections without device identity.
