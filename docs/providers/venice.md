---
summary: "Use Venice AI privacy-focused models in Agent-I"
read_when:
  - You want privacy-focused inference in Agent-I
...
```bash
Agent-I onboard --auth-choice venice-api-key
```
...
```bash
Agent-I agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, Agent-I shows all available Venice models. Pick based on your needs:
...
```bash
Agent-I models set venice/claude-opus-45
Agent-I models set venice/llama-3.3-70b
```

List all available models:

```bash
Agent-I models list | grep venice
```

## Configure via `Agent-I configure`

1. Run `Agent-I configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Agent-I automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
Agent-I agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
Agent-I agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
Agent-I agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
Agent-I agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
Agent-I agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `Agent-I models list` to see currently available models. Some models may be temporarily offline.
