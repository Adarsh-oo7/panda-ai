...
- Publish as `Agent-I/plugin-sdk` (or export from core under `Agent-I/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `Agent-IPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: Agent-IConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: Agent-IConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: Agent-IConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: Agent-IConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: Agent-IConfig): string;
  };
};
```
...
- Introduce `Agent-I/plugin-sdk`.
- Add `api.runtime` to `Agent-IPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `Agent-IRuntime: ">=2026.2.0"`).
