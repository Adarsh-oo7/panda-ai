...
4. Users browse, download, and install skills in Agent-I.
...
If you want to add new capabilities to your Agent-I agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
4. Start a new Agent-I session so it picks up the new skill.
...
## How it fits into Agent-I

By default, the CLI installs skills into `./skills` under your current working directory. If a Agent-I workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). Agent-I loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.Agent-I/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Agent-I how to perform a
...
Interested in becoming a moderator? Ask in the Agent-I Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Agent-I workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/Agent-I/skills` and `~/.Agent-I/skills`). This is designed to find older skill installs without extra flags.
