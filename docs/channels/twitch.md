...
read_when:
  - Setting up Twitch chat integration for Agent-I
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Agent-I connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
Agent-I plugins install @Agent-I/twitch
```

Local checkout (when running from a git repo):

```bash
Agent-I plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `Agent-I_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "Agent-I", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use Agent-I_TWITCH_ACCESS_TOKEN env var)
...
```bash
Agent-I_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
