...
Agent-I ships two lightweight web tools:
...
If no `provider` is explicitly set, Agent-I auto-detects which provider to use based on available API keys, checking in this order:
...
3. Run `Agent-I configure --section web` to store the key in config (recommended), or set `BRAVE_API_KEY` in your environment.
...
**Recommended:** run `Agent-I configure --section web`. It stores the key in
`~/.Agent-I/Agent-I.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.Agent-I/.env` (or your
service environment). See [Env vars](/help/faq#how-does-Agent-I-load-environment-variables).
...
environment. For a gateway install, put it in `~/.Agent-I/.env`.

If no base URL is set, Agent-I chooses a default based on the API key source:
...
For a gateway install, put it in `~/.Agent-I/.env`.
