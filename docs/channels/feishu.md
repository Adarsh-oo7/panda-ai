...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Agent-I to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
Agent-I plugins install @Agent-I/feishu
```

Local checkout (when running from a git repo):

```bash
Agent-I plugins install ./extensions/feishu
```
...
If you just installed Agent-I, run the wizard:
...
2. Configuring app credentials in Agent-I
...
1. You already ran `Agent-I channels add` for Feishu
2. The gateway is running (`Agent-I gateway status`)
...
```bash
Agent-I pairing approve feishu <CODE>
```
...
2. Run `Agent-I logs --follow` and look for `chat_id`
...
2. Run `Agent-I logs --follow` and look for `open_id`
...
| `Agent-I gateway restart` | Restart gateway service       |
| `Agent-I logs --follow`   | Tail gateway logs             |
...
4. Check logs: `Agent-I logs --follow`
...
5. Ensure the gateway is running: `Agent-I gateway status`
6. Check logs: `Agent-I logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.Agent-I/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.Agent-I/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
