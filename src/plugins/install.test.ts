...
function setupManifestInstallFixture(params: { manifestId: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "agenti.plugin.json"),
    JSON.stringify({
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "agenti.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromDir", () => {
  ...
  it("uses agenti.plugin.json id as install key when it differs from package name", async () => {
